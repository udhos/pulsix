// Package main provides an end-to-end benchmark tool for Pulsix Stream API.
package main
...
func main() {
	...
	injector := streaminject.New(streaminject.Options{
		StreamOptions: stream.Options{
			...
			FlushThresholdSilence: cfg.flushSilence,
			InboxSize:             cfg.inboxSize,
		},
		BufferSize: cfg.injectBuffer,
		OnAck: func(_ string) {
	...
	for i := range cfg.messages {
		...
		select {
		case injector.C <- streaminject.InjectMessage{Receipt: strconv.Itoa(i), Data: payload}:
...
func parseFlags() configFlags {
	...
	flag.DurationVar(&cfg.flushAge, "flush-age", stream.DefaultFlushThresholdAge, "stream flush threshold by age")
	flag.Int64Var(&cfg.flushBytes, "flush-bytes", stream.DefaultFlushThresholdBytes, "stream flush threshold by bytes")
	flag.DurationVar(&cfg.flushSilence, "flush-silence", 0, "stream flush threshold by silence")
	flag.IntVar(&cfg.inboxSize, "inbox-size", stream.DefaultInboxSize, "stream inbox channel size")
	flag.BoolVar(&cfg.disableMsgID, "disable-message-id", false, "disable metadata message ID generation")

	flag.IntVar(&cfg.injectBuffer, "inject-buffer", streaminject.DefaultBufferSize, "injector channel buffer size")
...
func validateFlags(cfg configFlags) error {
	...
	if cfg.inboxSize <= 0 {
		return fmt.Errorf("-inbox-size must be > 0")
	}
...
func buildBackend(ctx context.Context, cfg configFlags) (benchBackend, error) {
	...
	case "sim":
		...
		createdTemp := false
		if baseDir == "" {
			d, err := os.MkdirTemp("", "pulsix-stream-bench-")
			if err != nil {
...
func report(cfg configFlags, elapsed time.Duration, counters *runCounters, lat *latencyStats) {
	...
	fmt.Println("=== pulsix-stream-bench report ===")
